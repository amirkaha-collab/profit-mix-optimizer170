...
def compute_baseline(case: AdvisoryCase, df_long=None) -> bool:
    """
    Build case.current_snapshot from the legacy baseline when available.
    If the legacy baseline is missing, derive it from the current holdings / pf_holdings
    so the wizard and the older portfolio views stay in sync.
    ...
    try:
        ...
        # 1) Preferred path: existing engine result already in session_state.
        if not bl and case.current_snapshot and case.current_snapshot.raw:
            bl = dict(case.current_snapshot.raw)
            total = float(case.current_snapshot.total_value or total or 0)

        # 2) Fallback: derive a weighted baseline from the portfolio tables that already
        #    render correctly in the legacy UI (pf_holdings / case holdings).
        if not bl:
            holdings = st.session_state.get("pf_holdings") or case.all_holdings or []
            if holdings:
                try:
                    import pandas as pd
                    ...
                    total = float(totals.get("total") or total or 0)
                    if total > 0:
                        mgr_breakdown = {}
                        if "provider" in df.columns:
                            active = df[~df["excluded"].astype(bool)].copy()
                            active = active[active["amount"].notna()]
                            if not active.empty:
                                by_mgr = active.groupby("provider")["amount"].sum()
                                mgr_breakdown = {
                                    str(k): float(v) for k, v in by_mgr.items() if pd.notna(v)
                                }
                        bl = {
                            "amount": total,
                            "stocks": totals.get("equity"),
                            "foreign": totals.get("foreign"),
                            "fx": totals.get("fx"),
                            "illiquid": totals.get("illiquid"),
                            "sharpe": totals.get("sharpe"),
                            "manager_breakdown": mgr_breakdown,
                            "n_products": int(totals.get("n_products") or 0),
                            "n_managers": int(totals.get("n_managers") or 0),
                        }
        ...
        # 3) Last fallback: compute from holdings using the shared engine if available.
        if not bl and case.all_holdings and df_long is not None:
            try:
                ...
                if bl:
                    total = float(bl.get("amount", total) or total or 0)
                    st.session_state["portfolio_baseline"] = bl
                    st.session_state["portfolio_total"] = total
            except Exception:
                pass
        ...
        if bl:
            snap = PortfolioSnapshot.from_baseline_dict(bl, total)
            snap.n_products = int(bl.get("n_products") or len(st.session_state.get("pf_holdings") or case.all_holdings or []))
            snap.n_managers = int(bl.get("n_managers") or len({str(h.get("provider", "")) for h in (st.session_state.get("pf_holdings") or case.all_holdings or []) if h.get("provider")}))
            snap.manager_breakdown = dict(bl.get("manager_breakdown") or {})
            case.current_snapshot = snap
