...
def render_research_workbench(nav_to_fn) -> None:
    """Research mode — direct professional workbench."""
    _mode_header("🔭 מצב מחקר", "כלים לניתוח חופשי", "גישה ישירה — ללא סדר מחייב")

    TOOLS = [
        ("🏦", "בניית תמהיל",     "בחר עולם מוצר, הגדר יעדים, קבל חלופות",
         [("קרנות השתלמות","rb_ht"),("קופות גמל","rb_gm"),
          ("פוליסות חיסכון","rb_pl"),("קרנות פנסיה","rb_pn"),("גמל להשקעה","rb_gi")]),
        ("📐", "ניתוח מוסדי",     "אסטרטגיות אלוקציה היסטוריות של גופים מובילים",
         [("קרנות השתלמות","ri_ht"),("קופות גמל","ri_gm"),
          ("פוליסות חיסכון","ri_pl"),("קרנות פנסיה","ri_pn"),
          ("גמל להשקעה","ri_gi")]),
        ("📊", "ניתוח תיק לקוח", "ייבוא מסלקה, ניתוח חשיפות, גרפים ודוחות", None),
    ]
    c1, c2, c3 = st.columns(3, gap="medium")
    for col, (ico, title, desc, worlds) in zip([c1,c2,c3], TOOLS):
        with col:
            st.markdown(f"""
<div class="rb-card">
  <span class="rb-ico">{ico}</span>
  ...
  <div class="rb-desc">{desc}</div>
            ...
            if worlds:
                for w, k in worlds:
                    short = w.replace("קרנות ","").replace("קופות ","")
                    if st.button(short, key=k, use_container_width=True, type="secondary"):
                        # ISA research gets dedicated screen; mix-builder goes to app page
                        if title == "ניתוח מוסדי":
                ...
                if st.button("פתח ניתוח תיק", key="rb_pf",
                             use_container_width=True, type="primary"):
                    nav_to_fn("app", "תיק לקוח")
                    st.rerun()
...
def render_isa_research_screen(df_long, nav_to_fn) -> None:
    ...
    st.markdown(
        ...
        f'🔭 מחקר מוסדי</div>'
        f'<div style="font-size:17px;font-weight:900;color:#fff">'
        f'ניתוח אסטרטגיות מוסדיים — {product_type}</div>'
        f'</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    # ── Render the existing ISA engine directly ─────────────────────────────
    try:
