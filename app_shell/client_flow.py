...
def _step_snapshot(case, df_long, engine: WorkflowEngine) -> None:
    ...
    # Compute snapshot from the existing engine / synced legacy state.
    from case_management.before_after_pipeline import compute_baseline
    built = compute_baseline(case, df_long)

    if built and case.current_snapshot:
        # Persist immediately so the workflow engine and the legacy UI stop disagreeing.
        case.mark_step_done(STEP_SNAPSHOT)
        CaseStore.save(case)
        st.success("✅ תמונת מצב חושבה")
        _render_snapshot_summary(case.current_snapshot)
    else:
        st.warning("לא ניתן לחשב עדיין תמונת מצב. יש להשלים לפחות מוצר אחד עם סכום, ורצוי גם נתוני חשיפה.")
